...
var url = 'mongodb://localhost:27017/PizzaShop';

MongoClient.connect(url, function (err, db) {
  if (err) throw err;

  var dbo = db.db('PizzaShop');
  dbo.createCollection('pizzas', function (error, res) {
  ...
  var pizzas = [
    {
      ...
      name: 'Pepperoni Fresh z pieprzem',
      types: [0, 1],
      sizes: [26, 30, 40],
      price: 803,
      category: 0,
  ...
  dbo.collection('pizzas').insertMany(pizzas, function (error, res) {
    if (error) throw error;
    console.log('Pizzas inserted');
    db.close();
  });
